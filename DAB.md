...
> **TII (Transmitter Identification Information):** In real broadcast infrastructure, the null symbol is not completely silent — it carries a sparse comb of subcarriers encoding a TII signature that identifies individual transmitters in a Single Frequency Network (SFN). An RTL-SDR receiver typically treats the null symbol purely as a near-zero energy marker for frame synchronization and does not decode TII.
...
For Mode I, frequency de-interleaving follows two steps defined in ETSI EN 300 401 §14.6:

1. Generate the full permutation Π over 0…2047 using the LCG above.
2. Retain only values in the range **256…1792 excluding 1024**, preserving order, to form the sequence dₙ (n = 0…1535).
3. Map logical index n to carrier index: **k = dₙ − 1024**.

The de-interleaver applies this mapping to restore logical (coded-bit) order from the received carrier order.
...
**MSC decoding pipeline:**

```
Frequency De-interleave → Demultiplex CIFs → Time De-interleave (16 CIFs) → Depuncture → Viterbi
```

**Time interleaving** (MSC only — FIC skips this step): The MSC uses a 16-frame convolutional time interleaver. Bits from adjacent logical positions in one CIF are spread across 16 consecutive CIFs, covering 16 × 24 ms = 384 ms. This protects against burst errors and Rayleigh fading (e.g. signal dropouts in moving vehicles) by ensuring that a single burst affects only a small fraction of any one code block. The FIC is not time-interleaved so receivers can parse service metadata quickly.
...
> **Placement note:** DAB applies energy dispersal at the **transport layer** — at the transmitter, data is scrambled before convolutional encoding; at the receiver, de-dispersal is applied to the **hard bits output by the Viterbi decoder**, not at the RF or OFDM layer. This differs from physical-layer scrambling (e.g. Wi-Fi), where whitening is applied to the baseband bit stream directly.
...
FIGs are packed back-to-back within the 30-byte payload. Each FIG starts with a 1-byte header; the remaining bytes up to byte 29 that are not used by FIGs are filled with `0xFF` padding bytes (end-of-FIB marker). CRC-16 covers bytes 0–29 (polynomial 0x1021, init 0xFFFF, output complemented).
...
- **type** (bits 7–5): FIG type (0 = MCI/SI, 1 = labels, 5 = FIDC, 6 = CA; other values are reserved or context-specific)
...
MP2 encoder (DAB)  or  HE-AAC encoder (DAB+)
    │  e.g. 128 kbps MP2: 1,536 bytes per 96 ms DAB frame (4 × 384-byte MP2 frames, each 24 ms / 1,152 samples)
    │
    ▼  Channel coding
...
After energy de-dispersal (PRBS reset each CIF), the 672 bytes are MPEG-1 Layer II (MP2) audio data. For MPEG-1 Layer II at 48 kHz, each audio frame contains 1,152 PCM samples = 24 ms. This sub-channel delivers 672 bytes per 96 ms DAB frame, which at 56 kbps equals exactly 4 MP2 frames of 168 bytes each — one per CIF — aligning neatly with the 96 ms DAB frame boundary.
...
One 96 ms DAB frame:
  ...
  └─ 4 CIFs decoded  → 672 bytes MP2 audio data
                        → 4 complete MP2 frames (each 24 ms, 1,152 samples @ 48 kHz)
                        → 4 × 1,152 = 4,608 PCM stereo samples @ 48 kHz
                        → 96 ms of continuous audio output
```
...
A DAB+ audio super-frame is assembled from **5 consecutive CIFs** (each CIF is 24 ms, so one super-frame = 5 × 24 ms = **120 ms** of audio). With 96 bytes per CIF in this example:

```
5 CIFs × 96 bytes = 480 bytes of RS input
```

These bytes are arranged into RS codewords. The RS code is RS(120, 110, t=5) over GF(2⁸):

```
n_rs  = ceil(480 / 110) = 5 codewords  (5 × 110 = 550; pad 70 bytes with 0x00)
Input to RS encoder: 5 × 110 = 550 bytes (includes 70 bytes padding)
RS adds 5 × 10 = 50 parity bytes
Total RS block:  5 × 120 = 600 bytes transmitted
```

After RS correction at the receiver, the 70 padding bytes are discarded:

```
RS decode: 600 bytes → 550 bytes → strip 70 padding → 480 bytes super-frame payload
...
Remaining super-frame bytes are packed HE-AAC **access units**. Each AU is preceded by a length field in a header table at the start of the super-frame. With a 24 kHz AAC core rate and SBR enabled, a DAB+ superframe contains **3 AUs**, each encoding 960 samples at the 24 kHz core rate (= 40 ms per AU):

```
Super-frame (480 bytes):
  ├─ 3 bytes:  Fire code header
  ├─ 2 × 2 bytes:  AU length table  (2 entries; last AU length inferred)
  └─ AUs packed back-to-back:
       AU 0:  ~156 bytes  (HE-AAC frame, 960 core samples → 40 ms)
       AU 1:  ~157 bytes
       AU 2:  ~156 bytes  ← last AU length inferred from super-frame size
       (each AU ends with a 2-byte AU CRC)
```

3 AUs × 960 core samples @ 24 kHz = 2,880 core samples; SBR doubles to **5,760 samples @ 48 kHz** = **120 ms** of audio per super-frame. Number of AUs per superframe depends on AAC core sampling rate: 2 AUs at 16 kHz core, **3 AUs at 24 kHz core** (with SBR), 4 AUs at 32 kHz, 6 AUs at 48 kHz.
...
AU bytes → HE-AAC v2 decoder (core AAC-LC + SBR + PS):
  Core AAC-LC:  decodes 960 samples @ 24 kHz core rate
  SBR:          spectral band replication → 1,920 samples @ 48 kHz
...
```
Five consecutive CIFs (5 × 24 ms = 120 ms) → one DAB+ audio super-frame:
  ├─ FIC decoded each transmission frame → ensemble metadata (same as DAB)
  └─ 5 CIFs assembled
       → RS(120,110) outer FEC corrects burst errors
       → Fire code sync confirmed
       → 3 HE-AAC AUs decoded (24 kHz core + SBR)
       → 5,760 stereo samples @ 48 kHz  = 120 ms audio output
```

Compared to DAB, the RS outer code provides an additional error-correction layer on top of Viterbi, making DAB+ significantly more robust at the same transmit power. The tradeoff is a minimum latency of one full super-frame (120 ms) before the first audio can be decoded.
